...
class Registration
{
    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat, $captcha)
    {
		...
		if(empty($user_name) || empty($user_name) || empty($user_password) || empty($user_password_repeat) || empty($captcha)) {
			$this->errors[] = "Potrebno je ispuniti sva polja!";
		}
        elseif (strtolower($captcha) != strtolower($_SESSION['captcha'])) {
            $this->errors[] = "Pogrešno prepisana riječ sa slike.";
        } elseif (empty($user_name)) {
            $this->errors[] = MESSAGE_USERNAME_EMPTY;
        } elseif (empty($user_password) || empty($user_password_repeat)) {
            $this->errors[] = MESSAGE_PASSWORD_EMPTY;
        } elseif ($user_password !== $user_password_repeat) {
            $this->errors[] = "Netočna potvrda lozinke.";
        } elseif (strlen($user_password) < 6) {
            $this->errors[] = MESSAGE_PASSWORD_TOO_SHORT;
        } elseif (strlen($user_name) > 64 || strlen($user_name) < 2) {
            $this->errors[] = "Neodgovarajuće korisničko ime.";
        } elseif (!preg_match("/^(?=.{2,64}$)[a-zA-Z][a-zA-Z0-9]*(?: [a-zA-Z0-9]+)*$/", $user_name)) {
            $this->errors[] = "Neodgovarajuće korisničko ime.";
        } elseif (empty($user_email)) {
            $this->errors[] = MESSAGE_EMAIL_EMPTY;
        } elseif (strlen($user_email) > 64) {
            $this->errors[] = "Nevažeća e-mail adresa.";
        ...
        // finally if all the above checks are ok
            ...
            $query_check_user_name = $this->db_connection->prepare('SELECT user_name, user_email FROM users WHERE user_name=:user_name OR user_email=:user_email COLLATE utf8_bin');
            $query_check_user_name->bindValue(':user_name', $user_name, PDO::PARAM_STR);
            $query_check_user_name->bindValue(':user_email', $user_email, PDO::PARAM_STR);
            $query_check_user_name->execute();
            $result = $query_check_user_name->fetchAll();
			
//CHECK 			$sql = "SELECT * FROM users WHERE user_name = '" . $user_name . "' OR user_email = '" . $user_email . "';";
//CHECK            $query_check_user_name = $this->db_connection->query($sql);
//CHECK			
...
//            }
            ...
            if (count($result) > 0) {
                for ($i = 0; $i < count($result); $i++) {
                    $this->errors[] = "Zauzeto APPet ime ili e-mail adresa.";
                }
            } else {
