 ...
 * ZA POTREBE APLIKACIJE APPet UVEDENE SU IZMJENE - originalna verzija priložena i naznačena naknadno
 */
class Login
{
    /**
    ...
    public function __construct()
    {
        ...
        // login with cookie - NIJE KORIŠTENO ZA APPet
        ...
        // zahtjev za promjenom lozinke u slučaju da je zaboravljena
        ...
        // mogućnost uploadanja fotografija nije korištena za APPet
    ...
    /**
     * Funkcija za povezivanje s bazom - za APPet korištena i vlastita funkcija u istu svrhu
     * @return bool status o uspješnosti
     */
    private function databaseConnection()
    {
        ...
        if ($this->db_connection != null) {
            ...
            try {
                ...
                $this->errors[] = "Neuspjelo povezivanje s bazom: " . $e->getMessage();
            }
        }
        // default return
        return false;
    ...
    private function getUserData($user_name)
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE user_name = :user_name COLLATE utf8_bin');
    ...
    /**
     * Logs in via the Cookie - Kod za zapamćivanje nije korišten za APPet
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email FROM users WHERE user_id = :user_id
                                                      AND user_rememberme_token = :user_rememberme_token AND user_rememberme_token IS NOT NULL COLLATE utf8_bin");
    ...
    private function loginWithPostData($user_name, $user_password, $user_rememberme)
    {
        if(empty($user_name) AND empty($user_password)) {
		  $this->errors[] = "Ispunite obrazac.";
		} elseif (empty($user_name)) {
            $this->errors[] = "Niste upisali APPet ime.";
        } else if (empty($user_password)) {
            $this->errors[] = "Niste upisali lozinku.";
        ...
        // if POST data (from login form) contains non-empty user_name and non-empty user_password
            ...
            // if user has typed a valid email address, we try to identify him with his user_email
                ...
                // database query, getting all the info of the selected user
                $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE user_email = :user_email COLLATE utf8_bin');
            ...
            if (! isset($result_row->user_id)) {
                ...
                $this->errors[] = "Neuspjela prijava.";
            } else if (($result_row->user_failed_logins >= 3) && ($result_row->user_last_failed_login > (time() - 30))) {
                $this->errors[] = "3 puta pogrešna lozinka. Pričekajte 30 sekundi.";
    ...
    /**
     * Edit the user's email, provided in the editing form - Za APPet korištena vlastita modificirana funkcija
     */
    ...
    /**
     * Edit the user's password, provided in the editing form - Za APPet korištena vlastita modificirana funkcija
     */
    public function editUserPassword($user_password_old, $user_password_new, $user_password_repeat)
    {
        ...
        // all the above tests are ok
            ...
            if (isset($result_row->user_password_hash)) {
                ...
                } else { // now it gets a little bit crazy: check if we have a constant HASH_COST_FACTOR defined (in config/hashing.php),
        ...
        if (empty($user_name)) {
            ...
            if (isset($result_row->user_id)) {
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->errors[] = "Neuspjelo povezivanje s bazom";
                }
            } else {
                $this->errors[] = "APPet ime ne postoji";
            }
        }
    ...
    public function sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash)
    {
        ...
        if(!$mail->Send()) {
            $this->errors[] = "Neuspjela izmjena lozinke: " . $mail->ErrorInfo;
            return false;
        } else {
            $this->messages[] = "Poslan e-mail za promjenu lozinke:";
            return true;
        }
    }
    ...
    public function checkIfEmailVerificationCodeIsValid($user_name, $verification_code)
    {
        ...
        if (empty($user_name) || empty($verification_code)) {
            $this->errors[] = "Neodstaju neki parametri";
        } else {
            // database query, getting all the info of the selected user
            $result_row = $this->getUserData($user_name);
            ...
            if (isset($result_row->user_id) && $result_row->user_password_reset_hash == $verification_code) {
                ...
                if ($result_row->user_password_reset_timestamp > $timestamp_one_hour_ago) {
                    ...
                    $this->errors[] = "Link za izmjenu je istekao";
                }
            } else {
                $this->errors[] = "APPet korisnik ne postoji";
            }
        }
    }
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        // is the repeat password identical to password
            ...
            $this->errors[] = "Pogrešna potvrda";
        // password need to have a minimum length of 6 characters
        } else if (strlen($user_password_new) < 6) {
            $this->errors[] = "Lozinka prekratka (min. 6 znakova)";
        // if database connection opened
        } else if ($this->databaseConnection()) {
            ...
            if ($query_update->rowCount() == 1) {
                ...
                $this->messages[] = "Uspješna promjena lozinke";
            } else {
                $this->errors[] = "Neuspjela promjena lozinke";
            }
        }
    }
